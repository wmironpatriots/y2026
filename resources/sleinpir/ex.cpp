...
constexpr double field_width = 8.069326;
constexpr double field_length = 16.540988;
...
constexpr double target_radius = 1.059942;
constexpr double cone_angle = std::numbers::pi / 4;  // rad
constexpr Eigen::Vector3d g{
  {0.0}, 
  {0.0}, 
  {9.806}
};  // m/s²
...
slp::VariableMatrix<double> f(const slp::VariableMatrix<double>& x) {
  ...
  constexpr double rho = 128.17;  // kg/m³
  auto v = x[slp::Slice{3, 6}, _];
  slp::Variable v2 = v.T() * v;
  auto v_norm = sqrt(v2);
  auto v_hat = v / v_norm;
  constexpr Eigen::Vector3d omega{{0.0}, {-2.0}, {0.0}};  // rad/s
  ...
  auto F_L = 0.5 * rho * v_norm * C_L * A;

  return slp::block<double>(
      {{v}, {-g - F_D / m * v_hat - F_L / m * cross(v, omega)}});
}
