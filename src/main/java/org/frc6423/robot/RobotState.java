...
public class RobotState {
  // TODO fill
  public static final Matrix<N4, N1> kPoseEstimateStdevs = new Matrix<>(Nat.N4(), Nat.N1());
   ...
   */
  public RobotState getInstace() {
    if (instance == null) instance = new RobotState();

    return instance;
  }

  private Pose3d previousOdoPose;
  private Pose3d odoPose;
  private Pose3d estPose;
  private final TimeInterpolatableBuffer<Pose3d> odoPoseBuffer =
      TimeInterpolatableBuffer.createBuffer(kBufferDuration);

  private final SwerveDriveKinematics kinematics;
  private SwerveModulePosition[] previousSwerveModulePoses;

  /** Create new {@link RobotState} */
  private RobotState() {
    // TODO get fromm drive constants
    ...
    // TODO get fromm drive constants
    previousSwerveModulePoses = new SwerveModulePosition[4];
  }
  ...
  public void addOdometryMeasurement(OdometryMeasurement sample) {
    ...
    Twist3d odoPoseTwist =
        toTwist3d(kinematics.toTwist2d(previousSwerveModulePoses, sample.swerveModulePoses()));
    previousSwerveModulePoses = sample.swerveModulePoses;
    odoPose = odoPose.exp(odoPoseTwist);

    // Utilize gyro measurements if present
    sample.gyroRotation3d.ifPresent(r -> odoPose = new Pose3d(odoPose.getTranslation(), r));

    // Add odometry sample of specified timestamp to odo buffer
    odoPoseBuffer.addSample(kBufferDuration, odoPose);

    // Calculate change in distance between odometry positions and apply to estimated pose
    Twist3d estPoseTwist = previousOdoPose.log(odoPose);
    estPose.exp(estPoseTwist);
  }

  public void addVisionMeasurement(VisionMeasurement measurement) {
    // exit if sample is too old or there are no recent odometry samples
    if (odoPoseBuffer.getInternalBuffer().isEmpty()
        || odoPoseBuffer.getInternalBuffer().lastKey() - kBufferDuration
            > measurement.timestampeSeconds) {
      return;
    }

    // Get odo sample at timestamp; exit if nonexistant
    var odoSample = odoPoseBuffer.getSample(measurement.timestampeSeconds);
    if (odoSample.isEmpty()) {
      return;
    }

    var odoToSample = new Transform3d(odoPose, odoSample.get());

    var estPoseAtTimestamp = estPose.plus(odoToSample);
    ...
    estPose = estPoseAtTimestamp.plus(scaledTransform).plus(transform.inverse());
  }
